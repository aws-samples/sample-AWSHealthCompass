...
def get_resources_by_identifier(untracked_resources: List[Dict[str, Any]], event: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    Args:
        untracked_resources: List of untracked resources with metadata
    ...
    try:
        ...
        for resource_item in untracked_resources:
            try:
                # Extract the actual ARN from the resource dictionary
                ...
                # Initialize the list for this identifier if it doesn't exist
                if identifier not in resources_by_identifier:
                    resources_by_identifier[identifier] = []

                # Add resource to the appropriate group with consistent format
...
def check_existing_event(event_arn: str, resource_arn: str) -> list:
    ...
    try:
        ...
        # First try direct lookup using the primary key
        try:
        ...
        # If direct lookup fails, try using GSI
        ...
        if items:
            ...
            # If still not found, try a scan as a last resort (not efficient but helpful for debugging)
...
def group_resources_by_tracking_status(event: Dict[str, Any]) -> Tuple[List[Dict[str, str]], List[str]]:
    ...
    try:
        ...
        # Log the resources we're processing
        ...
        for resource_id in resource_list:
            ...
            # Get entity details first so we have them regardless of tracking status
            ...
            if existing_tracking:
                ...
                # Add ticket information to the resource
                resource_info = {
                ...
                # Add ticket ID from tracking info
                for tracking_item in existing_tracking:
                    if 'ticketId' in tracking_item:
                        resource_info['ticket_id'] = tracking_item['ticketId']
                    if 'snowSysID' in tracking_item:
                        resource_info['ticket_id'] = tracking_item['snowSysID']
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    Args:
        event: AWS Health event from SQS
