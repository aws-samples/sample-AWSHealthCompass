...
Credential redaction — no token in any log statement.
TLS certificate verification must NOT be disabled.
Sanitize displayName from JIRA (strip HTML).
"""
...
# Strip HTML tags from user-controlled strings
...
def validate_connection(base_url: str, email: str, api_token: str) -> dict:
    ...
    Never logs api_token. Catches all exceptions and
    returns sanitized error dicts — no raw exception messages that might
    contain credentials.
    """
    url = f"{base_url}/rest/api/3/myself"

    # Build auth header — never log this value
    ...
    # Default urllib3 PoolManager verifies TLS certificates.
    ...
    try:
        # No redirects — valid JIRA Cloud does not redirect /myself
        ...
        # Catch-all — never expose raw exception message
    ...
    # Merge 401/403 into single error code
...
def _sanitize_display_name(value: str) -> str:
    ...
    displayName is user-controlled in JIRA.
    """
