"""Shared WAFv2 rule builder.

Region-agnostic helper that returns the ordered rule list used by BOTH
WebACLs so they stay in parity:

    p1  AWSManagedRulesCommonRuleSet        (override SizeRestrictions_BODY -> count)
    p2  AWSManagedRulesKnownBadInputsRuleSet
    p3  AWSManagedRulesAmazonIpReputationList
    p4  CompassRateLimit (per-IP rate-based)

WCU budget: 700 + 200 + 25 + 2 = ~927 (< 1500 cap). No AnonymousIpList / Bot
Control / Fraud Control.

Centralizes:
  * CRS `SizeRestrictions_BODY` -> `count` override (parity on both
    WebACLs) so a legit bulk-import body >8 KB is not falsely blocked.
  * `waf_mode` toggle: 'block' (default, enforcing) vs 'count'
    (observe-only; forces ALL managed-group override_actions AND the rate-rule
    action to `count`).
  * the API-edge custom 403 JSON block response is layered onto the
...
aws_cdk.aws_wafv2 is L1/Cfn-only (no stable L2).
"""
...
def build_waf_rules(*, waf_mode: str, rate_limit: int,
    ...
    Args:
        ...
        rate_limit: per-IP request threshold per 300s window.
        ...
        rate_rule_custom_response_key: WebACL custom_response_bodies key. When
            set AND waf_mode=='block', the rate rule emits the custom 403
            JSON. When None -> AWS-default 403 (CloudFront edge, Option A).
    """
    count_mode = waf_mode == "count"

    # count mode forces all managed groups to observe-only.
    group_override = (
    ...
    # never disable CRS; override only SizeRestrictions_BODY -> count
    ...
    # p4 — per-IP rate-based rule. Own rule -> `action`, not override_action.
    if count_mode:
        # observe-only. No custom response (count doesn't block).
        rate_action = wafv2.CfnWebACL.RuleActionProperty(count={})
    elif rate_rule_custom_response_key:
        # API edge only: custom 403 JSON + CORS header so the
